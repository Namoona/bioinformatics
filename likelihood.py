...
def viterbiToGetMostProbablePath(string, alphabet, states, transition, emissionProb):
   ...
   for i in range(len(states)):
       curProb[i][0] = (float(1/len(states)))
   ...
   for i, k in enumerate(string):
      ...
      if i == 0:
          for s in range(len(states)):
             curProb[s][i] = curProb[s][i] * (emissionProb[s][ind1])
          ...
          for s1 in range(len(states)):
             ...
             for s2 in range(len(states)):
                 probs.append(curProb[s2][i-1] * float((transition[s2][s1])) * float((emissionProb[s1][ind1])))
             # print probs
             # print i
             # print s1
             # print curProb
             curProb[s1][i] = sum(probs)
...
def main():
    f = open('/Users/tingliu/Downloads/dataset_11594_8.txt', 'r')
